...
def main():
    ...
    while True:
        try:
            ...
            # Job D: SmartW VHKT Morning Poll (Once daily at 07:10 AM)
            if now.hour == 7 and now.minute == 10 and last_run.get("smartw_vhkt_key") != today_str:
                last_run["smartw_vhkt_key"] = today_str
                run_job("smartw_vhkt", [python_exe, os.path.join(current_dir, "smartw_worker.py"), "--job", "vhkt"])

            # Job E: SmartW Periodic 2-Hour Report Review (Every 2 hours at minute :25 of even hours, including 12:25 PM)
            ...
            # Job H: Telegram Daily Report (Once daily at 07:30 AM)
            if now.hour == 7 and now.minute == 30 and last_run.get("daily_report_key") != today_str:
                last_run["daily_report_key"] = today_str
                run_job("daily_report", [python_exe, os.path.join(current_dir, "daily_report.py")])

            # Job I: Telegram Weekly Report (Once weekly on Mondays at 07:45 AM)
            if now.weekday() == 0 and now.hour == 7 and now.minute == 45 and last_run.get("weekly_report_key") != today_str:
