...
export default function Layout() {
  ...
  const isMapPage = location.pathname === '/network-map';

  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans">
      <Header />
      <main className={`flex-1 w-full overflow-x-hidden ${isMapPage ? 'p-0 md:p-6 lg:p-8' : 'p-4 md:p-6 lg:p-8'}`}>
        <div className={isMapPage ? 'w-full' : 'max-w-[1600px] mx-auto'}>
          <ErrorBoundary>
            <Outlet />
          </ErrorBoundary>
        </div>
