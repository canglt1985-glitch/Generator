...
def get_pretax_price(fuel_type: str, date_str: str = None) -> float:
    ...
    # Add path to import fuel_price correctly
    import sys
...
def get_station_info(site_id: str) -> dict | None:
    ...
    try:
        # Search by site_id or site_id_old
        ...
        return {
            ...
            'resolved_site_id': station.get("site_id") # Mã ID chuẩn hóa V2
        }
...
def update_incomplete_records(raw_data: list[dict]) -> int:
    ...
    updated = 0
    try:
        # Fetch V2 logs that are currently incomplete (missing gio_ket_thuc)
        ...
        for record in raw_data:
            ...
            # Match against cached incomplete logs
            matched_log = None
            for log in incomplete_logs:
                ...
                # Match by alarm ID or (site + date + start_time)
            ...
            # Update the incomplete log details
            if not duration_min:
...
def import_mfd_data(raw_data: list[dict]) -> dict:
    ...
    for record in unique_raw_data:
        ...
        # Resolve station info (helps mapping old site IDs to new V2 site IDs)
        ...
        if station_info:
            ...
            site_id = station_info['resolved_site_id'] # Use clean normalized V2 ID
        else:
        ...
        # If station not found, flag as pending for safety
        ...
        try:
            # Insert record to Supabase V2 generator_logs
            ...
            if res_insert.data:
                ...
                if status == 'approved':
                    ...
                    result['pending'] += 1
                    # Prepare info for Telegram alerts
    ...
    # Send Telegram Alerts after successful chèn
    try:
        import sys
        sys.path.append(os.path.dirname(current_dir))
        from bot_telegram_v2 import send_pending_log_alert
