...
from models import GeneralInfo, PowerSchedule, GeneratorLog, FuelLedger
...
def generate_weekly_report_data(target_date_str=None):
    ...
    # Ở đây chúng ta mượn hàm generate_daily_report_data từ daily_report để lấy số liệu MTD
    from daily_report import generate_daily_report_data
    
    if not target_date_str:
        target_date_str = datetime.now().strftime('%Y-%m-%d')
        
    # Lấy dữ liệu chung của MTD
    ...
    # Truy vấn thêm danh sách bất thường dài hạn
    from helpers import get_inactive_generators, get_weekly_anomaly_report, get_quarterly_fuel_anomalies, get_missing_logs_recommendations
    ...
    # Lấy danh sách cúp điện >= 3h thiếu log chạy máy từ đầu tháng đến nay
    try:
        from app import app
...
def format_weekly_report_message(data):
    ...
    """
    # Tính toán tổng mua Ledger cho MTD đối chiếu
    mtd_purchase_dau = (
...
def format_weekly_anomalies_message(data):
    ...
    # 1. Trạm máy nổ 90 ngày không chạy
    ...
    # 2. Tiêu hao bất thường quý (3 tháng)
    ...
    # 3. Đổ dầu nhiều lần không chạy máy
    ...
    # 4. Thiếu log chạy máy cho đợt cúp điện (Cúp >= 3h)
...
def send_weekly_report(target_date_str=None):
    ...
    Sends MTD statistics and weekly anomalies as two separate messages.
    """
    session = get_db_session()
...
if __name__ == "__main__":
    ...
    print("Testing Weekly Report calculation and send...")
    from app import app
    with app.app_context():
        send_weekly_report()
