...
for line in lines:
    ...
    if len(parts) >= 5:
        ma_qh = parts[0]
        if ma_qh in seen_codes:
            continue
        seen_codes.add(ma_qh)
        ...
        # Geofence logic for TVT3 (Eastern Dong Nai: Long Khánh, Thống Nhất, Cẩm Mỹ, Xuân Lộc, Định Quán, Tân Phú)
        # Excludes Bình Phước (Lat >= 11.35 and Lng <= 106.95)
        # Excludes Western Dong Nai (Bien Hoa, Long Thanh, Nhon Trach: Lng < 107.05 and Lat <= 10.85)
        is_tvt3 = False
        district = ""
        area_note = ""
        
        if nearest and min_dist <= 5000 and any(td in (nearest['huyen'] or '').lower() for td in ['cẩm mỹ', 'thống nhất', 'xuân lộc', 'long khánh', 'định quán', 'tân phú']):
            is_tvt3 = True
            district = nearest['huyen']
            area_note = f"Gần trạm {nearest['code']} ({nearest['xa']}, {district}) cách {round(min_dist)}m"
        elif lng >= 107.05 and lat <= 11.35:
            is_tvt3 = True
            if lat >= 11.0:
                district = "Định Quán / Tân Phú"
            elif lat >= 10.85:
                district = "Long Khánh / Thống Nhất / Xuân Lộc"
            else:
                district = "Cẩm Mỹ / Xuân Lộc"
            area_note = f"Vùng kinh độ {lng}°E thuộc TVT3"
        elif lat >= 10.90 and lat <= 11.30 and lng >= 107.00:
            is_tvt3 = True
            district = "Thống Nhất / Long Khánh"
            area_note = f"Vùng Thống Nhất / Long Khánh (TVT3)"
        else:
            is_tvt3 = False
            if lat >= 11.35 and lng <= 106.95:
                district = "Bình Phước"
            elif lng < 107.00:
                district = "Biên Hòa / Long Thành / Nhơn Trạch / Trảng Bom (TVT1/TVT2)"
            else:
                district = "Đơn vị ngoài TVT3"
        ...
        item_data = {
            ...
            'district': district,
            'area_note': area_note,
            'nearest_stn': nearest['code'] if nearest else '',
            'nearest_dist': round(min_dist) if nearest else 0
        }

        if is_tvt3:
            tvt3_list.append(item_data)
        else:
            other_list.append(item_data)

print(json.dumps({'tvt3_count': len(tvt3_list), 'other_count': len(other_list), 'tvt3_items': tvt3_list}, ensure_ascii=False, indent=2))
