...
for line in lines:
    ...
    if len(parts) >= 5:
        ...
        # Geofencing Dong Nai & TVT3 boundaries:
        # Dong Nai Province: Lat 10.45 to 11.45 (or 11.35), Lng 106.75 to 107.55
        # Bình Phước Province: Lat >= 11.35 and Lng <= 106.95 (Chơn Thành, Lộc Ninh, Bù Đăng, Đồng Xoài)
        # Tây Ninh / Bình Dương: Lng <= 106.75
        
        if lat >= 11.35 and lng <= 106.95:
            is_tvt3 = False
            district = "Bình Phước"
            reason = "Thuộc tỉnh Bình Phước"
        elif lng >= 107.05 and lat <= 11.35:
            # Eastern Dong Nai (TVT3): Long Khánh, Xuân Lộc, Cẩm Mỹ, Thống Nhất, Định Quán, Tân Phú
            is_tvt3 = True
            if lat >= 11.0:
            ...
            reason = f"Vùng địa bàn TVT3 (Đông Đồng Nai - Lng {lng}°E)"
        elif lat >= 10.90 and lat <= 11.30 and lng >= 107.00:
            is_tvt3 = True
            district = "Thống Nhất / Long Khánh"
            reason = f"Vùng địa bàn TVT3 (Thống Nhất - Long Khánh)"
        else:
            is_tvt3 = False
            if lng < 107.00:
                if lat <= 10.85:
                    district = "Long Thành / Nhơn Trạch (TVT2)"
                else:
                    district = "Biên Hòa / Trảng Bom / Vĩnh Cửu (TVT1)"
            else:
                district = "Đơn vị ngoài TVT3"
            reason = f"Tọa độ ({lat}, {lng}) thuộc khu vực Tây/Nam Đồng Nai"
...
tvt3_list = [r for r in results if r['is_tvt3']]
other_list = [r for r in results if not r['is_tvt3']]

print(f"TOTAL: {len(results)} | TVT3: {len(tvt3_list)} | OTHER: {len(other_list)}")
print(f"\n==========================================")
print(f"DANH SÁCH {len(tvt3_list)} TRẠM QUY HOẠCH THUỘC TVT3 DÀNH CHO TRÌNH KÝ")
print(f"==========================================")
for idx, r in enumerate(tvt3_list, 1):
    print(f"{idx:02d}. Mã QH: {r['ma_qh']}\t| VB: {r['vb']}\t| Vĩ độ: {r['lat']}\t| Kinh độ: {r['lng']}\t| Địa bàn: {r['district']}")
